...
Grok Build fresh runs default to `--always-approve` and `--sandbox workspace`. Workspace sandboxing is not full isolation: it may permit the workspace, `/tmp`, `~/.grok`, and documented network access. Configured arguments override defaults: `--permission-mode` may prompt or fail unattended runs; non-`plain` output can prevent direct completion-marker parsing; and `-p`/`--single` replaces Looper's generated task prompt. Daemon native resume and interactive `looper resume` takeover are unsupported for Grok Build; retries use a fresh checkpoint prompt, and Looper never uses ambient `--continue`.
