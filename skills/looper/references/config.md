...
- Forgejo projects require a `provider` and repo. Configure them in `[[projects]]`, or persist them with `looper project add --provider <id>` and restart `looperd`; the repo may be detected from a matching origin.
