...
func (r *Runner) runWriteSpecStep(ctx context.Context, input stepInput) (plannerCheckpoint, error) {
	...
	if !writeSpecCompleted {
		...
		prompt := buildPlannerPrompt(input.Project, issue, worktree, r.allowAutoPush, r.disclosure, r.agentRuntime, r.agentModel)
...
func buildPlannerPrompt(project storage.ProjectRecord, issue *checkpointIssue, worktree *checkpointWorktree, allowAutoPush bool, disclosureCfg config.DisclosureConfig, agentRuntime string, agentModel string) string {
	parts := []string{
