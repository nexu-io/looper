...
func TestRunPrepareWorktreeStepRealGatewayRecreatesMissingCommondir(t *testing.T) {
	...
	worktreeRoot := filepath.Join(root, "worktrees")
	branch := "feature/fix-42"
	projectID := "project_real_missing_commondir"
	if err := os.MkdirAll(worktreeRoot, 0o755); err != nil {
		t.Fatalf("MkdirAll worktreeRoot: %v", err)
	}
	gateway := gitinfra.New(gitinfra.Options{GitPath: "git"})
	...
	// Resolve linked private gitdir and strip commondir while leaving HEAD.
	gitMeta, err := os.ReadFile(filepath.Join(wtPath, ".git"))
	if err != nil {
		t.Fatalf("ReadFile .git: %v", err)
	}
	...
	// Probe must already report unusable: HEAD alone is insufficient without commondir.
	probeErr := errors.New("fatal: not a git repository (or any of the parent directories): .git")
	if localFixerWorktreeCheckoutUsable(wtPath) {
