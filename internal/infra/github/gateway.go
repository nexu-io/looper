...
func normalizeInlineReviewDisclosure(body string, disclosureCfg config.DisclosureConfig) string {
	if !strings.Contains(body, disclosure.Marker) && !strings.Contains(body, "Generated by Looper") {
		return body
	}
	if !disclosureCfg.Enabled || !disclosureCfg.Channels.ReviewComment {
		return stripInlineReviewDisclosure(body)
	}
	if disclosureCfg.Channels.InlineCommentVisible || !strings.Contains(body, "Generated by Looper") {
		return body
	}
