...
func (r *Runner) runValidation(ctx context.Context, input ValidationInput) (ValidationResult, error) {
	...
	if len(input.Commands) == 0 {
		return ValidationResult{Passed: true, Summary: "Validation skipped"}, nil
	}
	return ValidationResult{Passed: false, Summary: "Validation runner not configured", Output: strings.Join(input.Commands, "\n")}, nil
}
