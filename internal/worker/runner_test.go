...
func TestBuildWorkerPromptPlacesCustomInstructionsBeforeLifecycle(t *testing.T) {
	t.Parallel()
	cfg, err := config.Normalize(t.TempDir(), config.PartialConfig{Roles: map[string]config.PartialRoleConfig{"worker": {Instructions: stringPtr("Prefer small commits.")}}})
	if err != nil {
...
func TestBuildWorkerPromptOmitsDisabledCustomInstructions(t *testing.T) {
	t.Parallel()
	cfg, err := config.Normalize(t.TempDir(), config.PartialConfig{Instructions: &config.PartialInstructionsConfig{Enabled: testBoolPtr(false)}, Roles: map[string]config.PartialRoleConfig{"worker": {Instructions: stringPtr("Prefer small commits.")}}})
	if err != nil {
		t.Fatalf("Normalize() error = %v", err)
	}
	prompt, block, err := buildWorkerPromptWithInstructions("", "demo", cfg, workerInput{Repo: "acme/looper", Title: "fix bug", Branch: "looper/fix", BaseBranch: "main"}, nil, true, config.DefaultDisclosureConfig(), "")
