...
func TestCanonicalDomainsRoundTripAcrossStructuredFormats(t *testing.T) {
	...
	domains := map[string]func(Config) any{
		...
		"providers":     func(cfg Config) any { return cfg.Providers },
...
func canonicalRoundTripFixtureConfig() Config {
	...
	return Config{
		...
		Scheduler:     SchedulerConfig{PollIntervalSeconds: 45, MaxConcurrentRuns: 7, RetryMaxAttempts: 9, RetryBaseDelayMS: 1500, InfraRetryBudgetSeconds: 1800, SlowLaneWarnThresholdMS: 2500, DiscoveryCacheTTLSeconds: 12},
		...
		Providers: []ProviderConfig{{ID: "forgejo-main", Kind: ProviderKindForgejo, BaseURL: "https://forgejo.example.test", TokenEnv: stringPtr("FORGEJO_TOKEN")}},
