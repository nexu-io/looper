...
5. Establish concrete trigger → path → consequence evidence for each independent in-scope finding. Investigate uncertain candidates and omit unsupported claims from published findings. Group only the same root cause; keep unrelated concerns separate.
