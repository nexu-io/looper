...
func TestBuildReviewPromptIncludesActionableQualityContract(t *testing.T) {
	...
	for _, want := range []string{
		...
		"validate every inline review comment's `path`, `line`, `side`, `start_line`, and `start_side` against ANCHORABLE DIFF LOCATIONS",
		"Preserve exact anchors that fit those ranges",
...
func TestBuildReviewPromptIncludesAnchorableDiffLocations(t *testing.T) {
	...
	prompt := buildReviewPrompt("acme/looper", 42, reviewerCheckpoint{Snapshot: &checkpointSnapshot{Title: "Spec PR", HeadSHA: "abc123", PayloadJSON: string(payload)}}, "run_1", "reviewer:loop:abc123", false, false, config.DefaultDisclosureConfig(), "opencode", "")
