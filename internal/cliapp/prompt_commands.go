...
func previewReviewerLifecycleSafety(looperCLIPath string, disclosureCfg config.DisclosureConfig, agentRuntime string, agentModel string) string {
	minimalSeedContract := strings.Join([]string{
		...
		"Child agents must fetch PR details on demand with `gh`: use `gh pr view --json number,state,isDraft,baseRefName,headRefName,headRefOid,url` for metadata and drift checks; `gh pr diff --name-only` before scoped diffs; `gh pr diff -- <path>` for relevant files only; paginated `gh api repos/{owner}/{repo}/pulls/{number}/comments`, `gh api repos/{owner}/{repo}/pulls/{number}/reviews`, and `gh api repos/{owner}/{repo}/issues/{number}/comments` for review feedback; and `gh pr checks` only when CI status matters.",
