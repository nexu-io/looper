		...
		subcommands: []*cobra.Command{
			...
			newCommand(commandSpec{
				...
				persistentFlags: []flagSpec{
					...
					stringFlag("provider", "id|type", "Provider id or type for non-GitHub projects (forgejo, plane, github, or a configured provider id)"),
					stringFlag("snapshot-mode", "mode", "Snapshot mode for project add: async, full, or off"),
				},
				exampleLines: []string{
					"$ looper project list",
					"$ looper project add /path/to/repo",
					"$ looper project add /path/to/forgejo-repo --provider forgejo --repo owner/name",
