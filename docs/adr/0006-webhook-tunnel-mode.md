...
- New hook URLs include a `looper_mount` query value derived from the local per-repo HMAC secret. If the SQLite hook record is lost but the local secret file survives, `looperd` may re-record exactly one remote hook with that mount value before returning to hook-id authority. This is a narrow recovery path for Looper-created hooks; a plain URL, hook name, event set, or user-supplied id still never authorizes adoption.
...
- Before any mutating call on a recorded hook, `looperd` re-reads the hook by id and verifies that `config.url` still matches either the persisted managed URL, the current managed URL, or the legacy unmarked URL for that repo during one-time marker migration. This is a drift guard, not ownership proof; matching a plain URL alone never authorizes adopting or deleting a hook.
...
- `looperd` never deletes a remote hook it did not create. There is no inference from plain URL, name, events, or any other field.
...
- Daemon start: for each repo with `mode: tunnel`, ensure the locally recorded hook exists. If no local hook record exists, `looperd` first looks for exactly one mount-marked hook derived from the surviving local secret and re-records it; otherwise it creates a new hook and stores the new id. If a recorded `hook_id` returns `404`, create a replacement and store the new id. If the hook is disabled and still matches the managed URL, re-enable it, subject to a repeated-disable latch: after N consecutive auto-disables within a window, `looperd` stops re-enabling and surfaces `degraded` until the operator intervenes.
...
- The hook's `config.url` is `{public_base_url}/webhook/{owner}/{repo}?looper_mount=<secret-derived-id>`. The path encodes routing authority; payload fields are not used for routing. The query marker is ignored by the HTTP router and exists only to make Looper-created hook mounts recoverable without adopting plain URL matches.
...
- Adopting an existing hook by plain URL, name, events, or user-supplied hook id without a local creation record, because that collapses back into remote-state inference rather than Looper-created authority. The only automatic recovery exception is an exact `looper_mount` match derived from the surviving local secret file.
