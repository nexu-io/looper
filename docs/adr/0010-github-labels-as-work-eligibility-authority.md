...
The Network design introduces a `loopernet` cloud service holding registry, lease, and audit-log state. A natural temptation is to make the cloud the source of truth for routing decisions: "node red is assigned issue #42 because the cloud's audit log says so." This would parallel the kind of central dispatch service common in distributed work-queue systems.
...
- **GitHub work-intent state** is the source of truth for **work eligibility**. Worker intent is `looper:worker-ready`; Reviewer intent is an existing GitHub review request to a Network member. A Role claims work because this GitHub state exists, not because the cloud audit log records a routing decision.
...
- **The Lease row** in the `loopernet` database is the source of truth for **leadership**. The Router holds it or does not; if expired, it cannot act. Validated at side-effect boundaries per ADR-0007.
...
These Authorities never overlap. A Role asking "should I claim this work?" reads GitHub work intent and exact target labels. The Router asking "may I act?" checks the Lease. An operator asking "why did red get this issue?" reads the audit log.
...
- The reconciliation poll (Q8c in the design) is a correctness backstop, not a latency optimisation. If the cloud-side webhook path fails, Router reading GitHub labels directly catches up. The audit log might miss the entry, but eligibility is intact.
