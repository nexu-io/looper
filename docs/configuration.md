...
GitHub issue assignees, so worker and planner skip that GitHub claim step for
App identities and `[bot]` logins. Plane work-item assignment still runs.
Forgejo bot tokens must support account lookup (`read:user`)
...
- `providers[].maxResponseBytes` optionally caps Forgejo/tea API response bodies, including PR diffs. `0` (the default) is unlimited; a positive value is a hard byte cap.
