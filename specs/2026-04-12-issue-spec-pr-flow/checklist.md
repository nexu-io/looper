...
- [ ] 明确 Worker push 后如需 Reviewer 自动介入，由 Worker 主动补 reviewer
- [ ] 明确 v1 是否实现 `looper:needs-human` label（建议实现，作为兜底）
- [ ] 若 v1 不加 `looper:needs-human` label，确认内部 `manual_intervention` / `paused` 路径可直接复用
...
- [ ] 增加 Planner 启动入口（API）
- [ ] 明确是自动 discover 驱动、手动触发，还是两者兼容
- [ ] 明确 Phase 4 仅暴露手动触发入口；自动 discover 驱动在完整链路验收前不默认启用
- [ ] 如需 CLI，增加对应 planner 命令或最小触发入口
- [ ] 在状态接口 / 列表接口中暴露 planner loop 可见性
- [ ] 为 API / CLI 入口补齐测试

## Phase 5 - Reviewer 支持 Spec Review 阶段

- [ ] 在 reviewer `discoverPullRequests` 中新增 label-based 发现分支（`looper:spec-reviewing`），与现有 review-request 发现并存
- [ ] 明确两个 discover 来源如何去重
- [ ] Reviewer prompt 增加 phase awareness
- [ ] 让 Reviewer 能识别当前审查对象是 spec 还是 implementation
- [ ] 为 spec-reviewing discover 与 phase-aware review 补齐测试

## Phase 6 - Fixer / Review Clean 后的 Label 切换

- [ ] 定义 spec review clean 条件：`unresolvedThreadCount === 0` 且无 `REQUEST_CHANGES` review（与 fixer 的 collectFixItems 逻辑对齐）
- [ ] 明确由 Reviewer、Fixer，还是共享 helper 执行 label 切换
- [ ] label 切换前复用现有 PR lock（`pr:<repo>:<prNumber>`），不新增独立 lock key
- [ ] 切换逻辑保证幂等
- [ ] 实现 `looper:spec-reviewing` -> `looper:spec-ready`
- [ ] 为 label 切换成功、重复切换、竞态冲突补齐测试

## Phase 7 - Worker 支持接管已有 Spec PR

- [ ] 在 Worker 或 runtime 中新增 `looper:spec-ready` PR 的 discover 逻辑
- [ ] 在 runtime 中接入 Worker 对 `looper:spec-ready` 的 discover 调用
- [ ] 在 `apps/looperd/src/worker/index.ts` 增加 `pull_request` target 模式
- [ ] 在 `prepare-work` 中确定当前模式：`create-pr` vs `push-existing`
- [ ] 在 `prepare-work` 中把 execution mode（`create-pr` | `push-existing`）写入 checkpoint，后续 step 通过 mode 分支
- [ ] 在 `prepare-worktree` 中复用 Fixer 的现有 PR branch worktree 模式
- [ ] 读取 PR body / metadata 中显式记录的 `specPath`
- [ ] 确保 PR 模式下 `readSpecBlock` 从 worktree 路径读取 spec，而非 `projectRepoPath`
- [ ] `execute` 基于 spec PR 中的 spec 继续实现
- [ ] Worker agent prompt 中明确指示不要修改 spec 文件（v1 先做 soft protection）
- [ ] `validate` 在 PR 模式下继续生效
- [ ] 将 `open-pr` 逻辑改为 target-sensitive：
  - [ ] `project` 模式保留 `open-pr`
  - [ ] `pull_request` 模式改为 push existing PR
- [ ] Worker 开始执行时移除 `looper:spec-ready`，避免重复认领
- [ ] Worker push 完成后不新增 implementation label，直接依赖现有 reviewer / review-request 机制
- [ ] 如需 Reviewer 自动介入，Worker push 后调用 `addReviewersToPullRequest`
- [ ] 为 Worker dual-mode 补齐 checkpoint / resume / push 路径测试

## Phase 8 - Implementation 阶段 Review 行为补全

- [ ] 让 implementation 阶段的 Reviewer 同样具备 phase awareness
- [ ] 将 v1 的 implementation 阶段最小状态机回填到 spec 与实现说明

## Phase 9 - 兜底与失败恢复

- [ ] 定义 review / fix 循环不收敛的判定阈值
- [ ] 明确连续失败后的降级策略
- [ ] 为 planner / reviewer / fixer / worker 的失败恢复路径补齐测试

## Phase 10 - 端到端验收

- [ ] 准备一条真实或仿真的 issue → spec PR → spec-ready → implementation 场景
- [ ] 验证 issue discover 只命中符合规则的 issue
- [ ] 验证 Planner 能稳定创建 spec 文件与 spec PR
- [ ] 验证 Reviewer / Fixer 能把 PR 从 `looper:spec-reviewing` 推进到 `looper:spec-ready`
- [ ] 验证 Worker 能在同一 PR 上继续实现并 push
- [ ] 验证 implementation 后仍能进入后续 review 流程
- [ ] 将 acceptance flow 结果回填到 spec 或补充单独验收记录
